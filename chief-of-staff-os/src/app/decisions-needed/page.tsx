...
import { fmtDate, isOverdue, daysAgo } from "@/lib/utils/date";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Unresolved items awaiting CEO / C-suite resolution."
        actions={<Link href="/decisions-needed/new" className="btn-primary">+ New</Link>}
      />

      <div className="mb-5 grid grid-cols-3 gap-3">
        <Stat label="Total open" value={items.length} />
        <Stat label="Due this week" value={dueThisWeek.length} accent={dueThisWeek.length > 0 ? "med" : undefined} />
        <Stat label="Overdue" value={overdue.length} accent={overdue.length > 0 ? "high" : undefined} />
      </div>

      {items.length === 0 ? (
        <EmptyState title="Nothing waiting" description="No open decisions needed." />
      ) : (
          ...
          <table className="w-full text-sm table-zebra">
            <thead className="border-b border-ink-200 text-left text-[11px] uppercase tracking-wide text-ink-500">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">Impact if delayed</th>
              ...
              {items.map((d) => (
                <tr key={d.id} className="border-b border-ink-100">
                  <td className="px-4 py-2.5">
                    ...
                    {d.recommendation ? (
                      <div className="mt-0.5 text-[11px] text-ink-500">Rec: {d.recommendation}</div>
                    ) : null}
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-[11px] text-ink-500">
                    {d.priority ? <>P: {d.priority.name} </> : null}
                    {d.project ? <>· Pj: {d.project.name} </> : null}
                    {d.meeting ? <>· M: {d.meeting.title}</> : null}
                  </td>
                </tr>
              ))}
            </tbody>
